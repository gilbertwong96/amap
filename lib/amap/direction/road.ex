defmodule Amap.Direction.Road do
  ...
  identical: the run's aggregated steps carry no `road`, while a path's own step has that
  field because the page's step table lists it — no run has printed a plain step's keys
  yet. The scalars keep the wire's form and arrive as strings, as v3's other distances and
  counts do. All seven entries in that run carried the same four keys.
  """
